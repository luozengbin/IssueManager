...
public class IssueSyncApp {
	...
	public void execute(SyncItem syncItem) throws IOException, InvalidFormatException {
		...
		try {
			...
			// エクセルの後から新規チケットを探し			
			if(syncItem.getKeyRowBeginIdx() <= 0 && syncItem.getKeyRowBeginIdx() > (sheet.getLastRowNum() + 1)) {
				log.error("読み込み開始位置が不正のため、処理を止める");
				return;
			}
			
			//for (int i = sheet.getLastRowNum(); i >= (syncItem.getKeyRowBeginIdx() > 0 ? (syncItem.getKeyRowBeginIdx() - 1) : 0); i--) {
			for (int i = syncItem.getKeyRowBeginIdx() - 1; i <= sheet.getLastRowNum(); i++) {
				// チケット管理番号の抽出
				row = sheet.getRow(i);

				if (row != null) {
					
					String keyNo = ExcelUtils.getCellContent(row.getCell(syncItem.getKeyColumnIdx() - 1), evaluator);
